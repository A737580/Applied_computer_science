...
from src.env import src_prefix
...
class TemplateMatchingWidget(QWidget):
    ...
    def selectSourceDir(self):
        dir_path = QFileDialog().getExistingDirectory(self, "Выбрать папку с изображениями")
        if dir_path:
    ...
    def selectTemplateDir(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Выбрать папку с шаблонами")
        if dir_path:
    ...
    def detectFaceParts(self):
        ...
        # Детекция лица
        face_cascade = cv2.CascadeClassifier(fr"{src_prefix}\haarcascade_frontalface_default.xml")
        ...
        # Детекция глаз
        eye_cascade = cv2.CascadeClassifier(fr"{src_prefix}\haarcascade_eye.xml")
