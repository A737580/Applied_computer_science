...
from gui.widgets.TestModelWidget import TestModelWidget
...
class PainterAttributionApp(QMainWindow):
    def __init__(self):
        ...
        self.tab_inference = TestModelWidget(self.shared_state)
    ...
    def select_root_directory(self):
        ...
        if dir_path:
            self.shared_state.root_dir = Path(dir_path)
            self.lbl_root_dir.setText(f"Выбрано: ...{str(self.shared_state.root_dir)[-40:]}")
            self.btn_load_genre.setEnabled(True)
            self.shared_state.selected_genre = None
            self.shared_state.class_names = []
            self.shared_state.class_to_idx = {}
            ...
            logging.info(f"Выбрана корневая директория: {self.shared_state.root_dir}")

    # --- Метод для кнопки "Загрузить жанр" ---
    @Slot()
    def load_genre_data(self):
        if not self.shared_state.root_dir:
            QMessageBox.warning(
        ...
        genres = (
            [d.name for d in self.shared_state.root_dir.iterdir() if d.is_dir()]
            if self.shared_state.root_dir.is_dir()
            else []
        )
        ...
        self.shared_state.selected_genre = genre_name
        genre_path = self.shared_state.root_dir / self.shared_state.selected_genre

        if not genre_path.is_dir():
            QMessageBox.warning(
                self,
                "Ошибка",
                f"Папка жанра '{self.shared_state.selected_genre}' не найдена в {self.shared_state.root_dir}.",
            )
            self.shared_state.selected_genre = None
            self.btn_train.setEnabled(False)
            self.btn_save_model.setEnabled(False)
        ...
        try:
            ...
            self.shared_state.class_names = [d.name for d in author_dirs]
            self.shared_state.class_to_idx = {name: i for i, name in enumerate(self.shared_state.class_names)}

            if not self.shared_state.class_names:
                QMessageBox.warning(
                    self,
                    "Внимание",
                    f"В папке жанра '{self.shared_state.selected_genre}' не найдено папок авторов.",
                )
                self.list_authors.clear()
                self.btn_train.setEnabled(False)
            ...
            self.list_authors.clear()
            self.list_authors.addItems(self.shared_state.class_names)
            self.lbl_selected_genre.setText(
                f"Загружен жанр: {self.shared_state.selected_genre} ({len(self.shared_state.class_names)} авторов)"
            )
            # Активируем кнопки обучения и загрузки/оценки
            self.btn_train.setEnabled(True)
            self.btn_save_model.setEnabled(False)
            self.lbl_train_status.setText(
                f"Статус: Данные жанра '{self.shared_state.selected_genre}' готовы."
            )
            self.btn_show_samples.setEnabled(False)
            self.clear_sample_images()
            self.clear_all_results_and_plots()
            logging.info(
                f"Загружен жанр '{self.shared_state.selected_genre}'. Авторы: {self.shared_state.class_names}"
            )
    ...
    def get_genre_from_user(self, available_genres):
        ...
        elif self.shared_state.selected_genre:
            return self.shared_state.selected_genre, True
        else:
            QMessageBox.warning(
                self,
                "Нет жанров",
    ...
    def show_author_samples(self):
        ...
        if not self.shared_state.root_dir or not self.shared_state.selected_genre:
            logging.warning("Не выбрана папка или жанр для показа примеров.")
            return

        author_path = self.shared_state.root_dir / self.shared_state.selected_genre / author_name
        try:
            image_files = (
    ...
    def start_training_process(self):
        ...
        if not self.shared_state.selected_genre or not self.shared_state.root_dir:
            QMessageBox.warning(
        ...
        params = {
            "root_dir": str(self.shared_state.root_dir),
            "genre": self.shared_state.selected_genre,
            "img_size": self.shared_state.current_img_size,
            "test_size": self.shared_state.current_test_size,
            "hidden_layers": (128,),
            "alpha": 1e-4,
            "random_state": self.shared_state.current_random_state,
            "max_iter": 300,
        }
    ...
    def display_training_results(self, results):
        ...
        self.shared_state.last_run_results = results
        self.shared_state.trained_model = results.get("model")
    ...
    def on_task_finished(self):
        ...
        self.set_ui_state(training=False, model_ready=(self.shared_state.trained_model is not None))
        # Очистка потока
        if self.thread:
            self.thread.quit()
            self.thread.wait()
    ...
    def set_ui_state(self, training: bool, model_ready: bool = False):
        ...
        self.btn_train.setEnabled(not training and self.shared_state.selected_genre is not None)
    ...
    def save_trained_model(self):
        ...
        if not self.shared_state.trained_model:  # Проверяем наличие модели
            QMessageBox.warning(
                self,
                "Модель не готова",
        ...
        model_to_save = self.shared_state.trained_model
        # Используем параметры из последнего запуска (обучения или оценки)
        params_to_save = self.shared_state.last_run_results.get("training_params", {})
        class_names_to_save = self.shared_state.last_run_results.get("class_names", [])

        model_dir = Path(f"{env.home_dir_prefix}/models")
        model_dir.mkdir(parents=True, exist_ok=True)
        genre_name = self.shared_state.selected_genre if self.shared_state.selected_genre else "unknown_genre"
    ...
    def clear_all_results_and_plots(self):
        ...
        self.clear_training_plots_and_info()
        self.clear_3d_plot()
        self.shared_state.last_run_results = {}
        self.shared_state.trained_model = None
        self.btn_save_model.setEnabled(False)
